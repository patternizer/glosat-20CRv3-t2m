...
def make_shell_command(year):
    ...
    job_id = 'SBATCH --job-name=max_anomaly.{0}\n'.format(year)      
#   job_normals = 'ncea ensemble-max/{1961..1990}_ensemble_max.nc ensemble_max_normals.nc\n'
    job_nco = 'ncdiff ensemble-max/{0}_ensemble_max.nc ensemble_mean_normals.nc {0}_ensemble_max_anomaly.nc\n'.format(year)
    job_file = 'run.{0}.max_anomaly.sh'.format(year)
...
if __name__ == "__main__":
    ...
    for year in range(year1,year2):
        file_out = '{0}_ensemble_max_anomaly.nc'.format(year)
