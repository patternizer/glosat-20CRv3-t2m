...
def make_shell_command(year):
    ...
    job_id = 'SBATCH --job-name=mean_anomaly.{0}\n'.format(year)      
#   job_normals = 'ncea ensemble-mean/{1961..1990}_ensemble_mean.nc ensemble_mean_normals.nc\n'
    job_nco = 'ncdiff ensemble-mean/{0}_ensemble_mean.nc ensemble_mean_normals.nc {0}_ensemble_mean_anomaly.nc\n'.format(year)
    job_file = 'run.{0}.mean_anomaly.sh'.format(year)
...
if __name__ == "__main__":
    ...
    for year in range(year1,year2):
        file_out = '{0}_ensemble_mean_anomaly.nc'.format(year)
